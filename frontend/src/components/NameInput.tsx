...
// Common Nigerian/African name prefixes for suggestions
const TITLES = ['Mr.', 'Mrs.', 'Miss', 'Dr.', 'Prof.', 'Engr.', 'Barr.'];
...
const NameInput: React.FC<Props> = ({ value, onChange, error, placeholder = 'Your full name' }) => {
  const [suggestions, setSuggestions] = useState<string[]>([]);

  const handleChange = (raw: string) => {
    onChange(raw);
    const word = raw.trim();
    if (word.length >= 1 && !raw.includes(' ')) {
      setSuggestions(TITLES.filter(t => t.toLowerCase().startsWith(word.toLowerCase())).map(t => `${t} `));
    } else {
      setSuggestions([]);
    }
  };

  const isValid = value.trim().split(' ').filter(Boolean).length >= 2;
  ...
  return (
    <div className="relative">
      ...
      {!error && isValid && <p className="text-green-600 text-xs mt-1 flex items-center gap-1"><CheckCircle className="w-3.5 h-3.5" />Looks good</p>}
    </div>
  );
};
